...
if [[ ! -z "$SECOR_MESSAGE_TIMESTAMP_NAME" ]]; then
        SECOR_CONFIG="$SECOR_CONFIG -Dmessage.timestamp.name=$SECOR_MESSAGE_TIMESTAMP_NAME"
    echo "message.timestamp.name=$SECOR_MESSAGE_TIMESTAMP_NAME"
fi
...
if [[ ! -z "$SECOR_MESSAGE_TRANSFORMER" ]]; then
        SECOR_CONFIG="$SECOR_CONFIG -Dsecor.message.transformer.class=$SECOR_MESSAGE_TRANSFORMER"
    echo "secor.message.transformer.class=$SECOR_MESSAGE_TRANSFORMER"
fi

if [[ ! -z "$SECOR_PARTITIONER_GRANULARITY_HOUR" ]]; then
        SECOR_CONFIG="$SECOR_CONFIG -Dpartitioner.granularity.hour=$SECOR_PARTITIONER_GRANULARITY_HOUR"
    echo "partitioner.granularity.hour=$SECOR_PARTITIONER_GRANULARITY_HOUR"
fi
